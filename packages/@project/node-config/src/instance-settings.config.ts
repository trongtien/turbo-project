...
import { Config, Env } from '@project/node-decorator';
...
export class InstanceSettingsConfig {
     ...
     */
    @Env('PROJECT_ENFORCE_SETTINGS_FILE_PERMISSIONS')
    ...
    constructor() {
        ...
        this.userHome = process.env.PROJECT_USER_FOLDER ?? process.env[homeVarName] ?? process.cwd();

        this.projectFolder = path.join(this.userHome, '.project');
    }
}
